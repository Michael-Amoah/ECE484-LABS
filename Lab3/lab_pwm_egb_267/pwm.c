...
void pwm_hw_setup()
{
	...
	DDRD|=(1<<PD3);  /* set OC2B = Arduino_Pin3 pin as output - TIMER 2 */
	DDRB|=(1<<PB3);  /* set OC1B = Arduino_Pin10 pin as output - TIMER 1 */
...
void pwm_with_pwm_hw()
{
	int step16 = 8192; // 1/8 of 16bit number
	int step8 = 32; // 1/8 of 8bit number
